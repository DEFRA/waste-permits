...
lab.experiment('Company Declare Offences tests:', () => {
  lab.experiment(`GET ${routePath}`, () => {
    ...
    const getDoc = async () => {
      ...
      Code.expect(doc.getElementById('submit-button').firstChild.nodeValue).to.equal('Save and continue')
      return doc
    }
    ...
    lab.test('success', async () => {
      ...
      Code.expect(doc.getElementById('relevant-offences-details').firstChild.nodeValue).to.equal(fakeApplication.relevantOffencesDetails)
  ...
  lab.experiment(`POST ${routePath}`, () => {
    ...
    lab.beforeEach(() => {
      postRequest = {
        ...
        headers: {},
        payload: {
          'offences': fakeApplication.relevantOffences,
          'relevant-offences-details': fakeApplication.relevantOffencesDetails
        }
      }
    ...
    lab.experiment('invalid', () => {
      ...
      lab.test('when offences not checked', async () => {
        postRequest.payload = {}
        await checkValidationMessage('offences', `Select yes if you have convictions to declare or no if you don't`)
      })

      lab.test('when offences set to yes and no details entered', async () => {
        postRequest.payload = {'offences': 'yes'}
        await checkValidationMessage('relevant-offences-details', 'Enter details of the convictions')
      })
    })
