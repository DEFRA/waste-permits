...
lab.experiment('ContactDetailService test:', () => {
  let sandbox
  ...
  lab.experiment('get', () => {
    lab.test('via id', async () => {
      const contactDetail = await ContactDetailService.get(context, { id: fakeAddressDetail.id })
      Code.expect(contactDetail.id).to.equal(fakeAddressDetail.id)
    })

    lab.test('via type', async () => {
      const contactDetail = await ContactDetailService.get(context, { type: fakeAddressDetail.type })
      Code.expect(contactDetail.id).to.equal(fakeAddressDetail.id)
    })
  })
  ...
  lab.experiment('save', () => {
    lab.test('via create', async () => {
      ...
      const contactDetail = new ContactDetailService(fakeContactDetail)
