...
const ScreeningToolCheck = require('../../../src/models/checkList/screeningTool.check')

const fakeScreeningTools = [
  { filename: 'FILENAME_1' },
  { filename: 'FILENAME_2' },
  { filename: 'FILENAME_3' }
]

const prefix = 'section-screening-tool'
...
lab.beforeEach(() => {
  ...
  sandbox.stub(BaseCheck.prototype, 'getUploadedFileDetails').value(() => fakeScreeningTools)
})
...
lab.experiment('Screening Tool Check tests:', () => {
  lab.experiment('buildlines', () => {
    let check
    let lines

    lab.beforeEach(async () => {
      check = new ScreeningToolCheck()
      lines = await check.buildLines()
    })

    lab.test(`(screening tool line) works correctly`, async () => {
      ...
      answers.forEach(({ answer, answerId }, answerIndex) => {
        ...
        const { filename } = fakeScreeningTools[answerIndex]
