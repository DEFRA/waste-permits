...
lab.experiment('Task List Model tests:', () => {
  ...
  lab.test('TaskList returned by getByApplicationLineId() method has the correct sections', async () => {
    ...
    Code.expect(taskList.sections[0].sectionItems[4].id).to.equal('give-contact-details')
    Code.expect(taskList.sections[0].sectionItems[5].id).to.equal('give-permit-holder-details')
    Code.expect(taskList.sections[0].sectionItems[6].id).to.equal('give-site-name-and-location')
    Code.expect(taskList.sections[0].sectionItems[7].id).to.equal('upload-the-site-plan')
    Code.expect(taskList.sections[0].sectionItems[8].id).to.equal('upload-technical-management-qualifications')
    Code.expect(taskList.sections[0].sectionItems[9].id).to.equal('tell-us-which-management-system-you-use')
    Code.expect(taskList.sections[0].sectionItems[10].id).to.equal('upload-the-fire-prevention-plan')
    Code.expect(taskList.sections[0].sectionItems[11].id).to.equal('confirm-the-drainage-system-for-the-vehicle-storage-area')
    Code.expect(taskList.sections[0].sectionItems[12].id).to.equal('confirm-confidentiality-needs')
    Code.expect(taskList.sections[0].sectionItems[13].id).to.equal('invoicing-details')
