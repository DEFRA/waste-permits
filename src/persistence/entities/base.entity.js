...
module.exports = class BaseEntity {
  ...
  static dynamicsToEntity (dynamicsData, customFilter = () => true) {
    ...
    // }
    //
    // And dynamicsToEntity is executed as follows:
    // Account.dynamicsToEntity(dynamicsData, ({dynamics}) => dynamics !== 'defra_date_day')
    //
    // Then the result would be:
    // {
  ...
  entityToDynamics (customFilter = () => true) {
    ...
    // }
    //
    // And entityToDynamics is executed as follows:
    // account.entityToDynamics(dynamicsData, ({dynamics}) => dynamics !== 'defra_age')
    //
    // Then the result would be:
    // {
    //    defra_id: '43acb872-19ca-e711-8112-5065f38a5b01',
    ...
    // - The dynamics account dynamicsEntity has been bound to the contacts dynamicsEntity via the contactId
    //
  ...
  static async listBy (context = {}, filterData = {}, orderByFields = '') {
    ...
    try {
      ...
      return response.value.map((result) => this.dynamicsToEntity(result))
    } catch (error) {
  ...
  async listLinked (context = {}, linked = this, filterData = {}, orderByFields = '') {
    ...
    try {
      const response = await dynamicsDal.search(query)
      return response.value.map((result) => linked.constructor.dynamicsToEntity(result))
    } catch (error) {
  ...
  async _deleteBoundReferences (dynamicsDal, fields) {
    ...
    //
    const { dynamicsEntity, mapping } = this.constructor
    const boundMapping = mapping
      .filter(({ bind }) => bind && bind.relationship) // only those fields that have a relationship with another dynamicsEntity
    ...
    if (boundMapping.length) {
      // select all the cleared references of entities bound to this dynamicsEntity and get their original values
  ...
  async save (context = {}, fields) {
    ...
    const dataObject = this.entityToDynamics(fieldsFilter)

    const { dynamicsEntity, readOnly } = this.constructor
    if (readOnly) {
      const errorMessage = `Unable to save ${dynamicsEntity}: Read only!`
      LoggingService.logError(errorMessage)
      throw new Error(errorMessage)
    }
  ...
  async delete (context = {}, id = this.id) {
    const { dynamicsEntity, readOnly } = this.constructor
    if (readOnly) {
      const errorMessage = `Unable to delete ${dynamicsEntity}: Read only!`
      LoggingService.logError(errorMessage)
      throw new Error(errorMessage)
    }
    const dynamicsDal = new DynamicsDalService(context.authToken)
  ...
  async link (context = {}, linked) {
    ...
    const query = `${dynamicsEntity}(${this.id})/${relationships[linkedModel]}/$ref`
